...
template <typename _index, typename _value,typename _seach>
class ValueComparableItem : public ComparableItem<_index,_value,_seach>{
private:
    // 私有构造函数
    explicit ValueComparableItem(boost::shared_ptr<ItemContainer<_index, _value>> _line)
            : ComparableItem<_index, _value,_seach>(_line) {}
public:

//    using ComparableItem<_index, _value>::ComparableItem;

    // 查询
    static boost::shared_ptr<ValueComparableItem<_index, _value,_seach>> createComparableItem(
            boost::shared_ptr<ItemContainer<_index, _value>> _line) {
        // 调用父类的构造函数来创建ValueComparableItem实例
        return boost::shared_ptr<ValueComparableItem<_index, _value,_seach>>(new ValueComparableItem<_index,_value,_seach>(_line));
    }

    virtual ItemContainer<_index, _value> query(const std::function<bool(const _seach&)>& condition) const {
    ...
    // 重载操作符 >
    virtual ItemContainer<_index, _value> operator>(const _seach& idx) const  {
        return query([&idx](const _seach& itemIdx) {
            return itemIdx > idx;
        });
    }
